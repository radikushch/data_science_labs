...
def ABF(Yin, iter):
    ...
    for j in range(1, iter):
        YoutAB[j] = Yextra + alfa * (Yin[j] - Yextra)
        Yspeed = Yspeed_retro + (beta / T0) * (Yin[j] - Yextra)
        Yspeed_retro = Yspeed
        Yextra = YoutAB[j] + Yspeed_retro
        alfa = (2 * (2 * j - 1)) / (j * (j + 1))
        beta = 6 / (j * (j + 1))
...
mYout20 = np.median(Yout20)
dYout00 = np.var(Yout00)
dYout10 = np.var(Yout10)
dYout20 = np.var(Yout20)
...
for i in range(iter):
    # Yin[i, 0] = float(quadraticS[i])   # -------- без шуму
    # Yin[i, 0] = float(quadraticSV[i])   # -------- нормальний шум
    Yin[i] = float(quadraticSAV[i])  # -------- нормальний та аномальний шум
...
Yout0AB = np.zeros(n)
for i in range(n):
    Yout0AB[i] = abs(YoutABG[i] - quadraticS[i])
# ------- гістограми вхідних похибок, МНК оцінок нормальних та аномальних  --------------
plt.hist(Yout20, bins=20, alpha=0.5, label='Yout20')
